...
local function expand_local(__x31)
  local ____id1 = __x31
  local __x32 = ____id1[1]
...
local function expand_function(__x34)
  ...
  local ____x37 = join({"%function", __args}, macroexpand(__body))
  drop(_G.environment)
  return ____x37
end
local function expand_definition(__x39)
  local ____id3 = __x39
  local __x40 = ____id3[1]
  ...
  local ____x42 = join({__x40, __name1, __args11}, macroexpand(__body1))
  drop(_G.environment)
  return ____x42
end
...
____x73.lua = "or"
____x72["or"] = ____x73
local infix = {____x61, ____x63, ____x64, ____x66, ____x67, ____x68, ____x70, ____x72}
...
local function getop(op)
  return find(function (level)
    local __x75 = level[op]
    if __x75 == true then
      return op
    else
      if is63(__x75) then
        return __x75[_G.target]
      end
    end
  end, infix)
end
...
local function compile_args(args)
  ...
  local ____x76 = args
  local ____i15 = 0
  while ____i15 < _35(____x76) do
    local __x77 = ____x76[____i15 + 1]
...
local function compile_special(form, stmt63)
  local ____id6 = form
  local __x78 = ____id6[1]
  local __args2 = cut(____id6, 1)
  local ____id7 = getenv(__x78)
...
local function op_delims(parent, child, ...)
  ...
  if __e18(precedence(__child), precedence(__parent)) then
    return {"(", ")"}
  else
    return {"", ""}
  end
...
function compile_function(args, body, ...)
  ...
  local __e19
  if __name3 then
    __e19 = compile(__name3)
  else
    __e19 = ""
  end
  local __id14 = __e19
  local __e20
  if _G.target == "lua" and __args4.rest then
    __e20 = join(__args4, {"|...|"})
  else
    __e20 = __args4
  end
  local __args12 = __e20
  local __args5 = compile_args(__args12)
  indent_level = indent_level + 1
  local ____x84 = compile(__body3, {_stash = true, stmt = true})
  indent_level = indent_level - 1
  local __body4 = ____x84
  ...
  if nil63(__form) then
    ...
    if special_form63(__form) then
      ...
      if atom63(__form) then
        __e24 = compile_atom(__form)
      else
        local __e25
        if infix63(hd(__form)) then
          __e25 = compile_infix(__form)
        else
          __e25 = compile_call(__form)
        end
        __e24 = __e25
      end
      local __form1 = __e24
...
local function lower_do(args, hoist, stmt63, tail63)
  local ____x91 = almost(args)
  local ____i17 = 0
  while ____i17 < _35(____x91) do
    local __x92 = ____x91[____i17 + 1]
    local ____y = lower(__x92, hoist, stmt63)
    if yes(____y) then
      local __e1 = ____y
...
local function lower_if(args, hoist, stmt63, tail63)
  ...
  if stmt63 then
    ...
    if is63(___else) then
      __e30 = {lower_body({___else}, tail63)}
    end
    return add(hoist, join({"%if", lower(__cond, hoist), lower_body({___then}, tail63)}, __e30))
  else
    local __e3 = unique("e")
    add(hoist, {"%local", __e3})
    local __e29
    if is63(___else) then
      __e29 = {lower({"%set", __e3, ___else})}
    end
    add(hoist, join({"%if", lower(__cond, hoist), lower({"%set", __e3, ___then})}, __e29))
    return __e3
  end
end
local function lower_short(x, args, hoist)
  ...
  if some63(__hoist1) then
    ...
    local __e31
    if x == "and" then
      __e31 = {"%if", __id19, __b4, __id19}
    else
      __e31 = {"%if", __id19, __id19, __b4}
    end
    return lower({"do", {"%local", __id19, __a3}, __e31}, hoist)
  else
...
local function lower_while(args, hoist)
  ...
  if none63(__pre) then
    __e32 = {"while", __c5, lower_body(__body5)}
  else
    __e32 = {"while", true, join({"do"}, __pre, {{"%if", {"not", __c5}, {"break"}}, lower_body(__body5)})}
  end
...
setenv("do", {_stash = true, special = function (...)
  ...
  local __s2 = ""
  local ____x133 = __forms
  local ____i18 = 0
  while ____i18 < _35(____x133) do
    local __x134 = ____x133[____i18 + 1]
    if _G.target == "lua" and immediate_call63(__x134) and "\n" == char(__s2, edge(__s2)) then
...
setenv("%if", {_stash = true, special = function (cond, cons, alt)
  ...
  indent_level = indent_level + 1
  local ____x135 = compile(cons, {_stash = true, stmt = true})
  indent_level = indent_level - 1
  local __cons = ____x135
  local __e33
  if alt then
    indent_level = indent_level + 1
    local ____x136 = compile(alt, {_stash = true, stmt = true})
    indent_level = indent_level - 1
    __e33 = ____x136
  end
  local __alt = __e33
...
setenv("%global-function", {_stash = true, special = function (name, args, body)
  if _G.target == "lua" then
    local __x143 = compile_function(args, body, {_stash = true, name = name})
...
setenv("%local-function", {_stash = true, special = function (name, args, body)
  if _G.target == "lua" then
    local __x146 = compile_function(args, body, {_stash = true, name = name, prefix = "local"})
...
setenv("error", {_stash = true, special = function (x)
  local __e35
  if _G.target == "js" then
    __e35 = "throw " .. compile({"new", {"Error", x}})
  else
    __e35 = "error(" .. compile(x) .. ")"
  end
  local __e7 = __e35
...
setenv("%object", {_stash = true, special = function (...)
  ...
  local __sep = __e41
  local ____x154 = pair(__forms2)
  local ____i20 = 0
  while ____i20 < _35(____x154) do
    local ____id28 = ____x154[____i20 + 1]
    local __k9 = ____id28[1]
    local __v9 = ____id28[2]
    __s5 = __s5 .. __c7 .. key(compile(__k9)) .. __sep .. compile(__v9)
    __c7 = ", "
    ____i20 = ____i20 + 1
  end
  return __s5 .. "}"
...
setenv("%literal", {_stash = true, special = function (...)
  ...
  local __s6 = ""
  local ____x156 = __args10
  local ____i21 = 0
  while ____i21 < _35(____x156) do
    local __x157 = ____x156[____i21 + 1]
    if string_literal63(__x157) then
      __s6 = __s6 .. _eval(__x157)
    else
      __s6 = __s6 .. compile(__x157)
    end
    ____i21 = ____i21 + 1
  end
  return __s6
...
setenv("%statement", {_stash = true, special = function (...)
  ...
  local __s7 = indentation()
  local ____x159 = __args111
  local ____i22 = 0
  while ____i22 < _35(____x159) do
    local __x160 = ____x159[____i22 + 1]
    if string_literal63(__x160) then
      __s7 = __s7 .. _eval(__x160)
    else
      __s7 = __s7 .. compile(__x160)
    end
    ____i22 = ____i22 + 1
  end
  __s7 = __s7 .. "\n"
